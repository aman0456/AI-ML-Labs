...
class SudokuSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        minval = min([len(state[x]) for x in state if len(state[x]) > 1])
        for s in state:
            if len(state[s]) == minval:
                for x in range(minval):
                    state1 = state.copy()
                    ret = assign(state1, s, state1[s][x])
                    if ret:
                        successors.append((ret, (s, state[s][x]), 1))
                break
